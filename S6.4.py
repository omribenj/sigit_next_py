...
def main():
    ...
    # for every 2 points draw the line, in the first and second array
    for i in range(0, len(first)-3, 4):
        draw.line((first[i], first[i+1], first[i+2], first[i+3]), fill=(255, 255, 0), width=10)
    for i in range(0, len(second)-3, 4):
        draw.line((second[i], second[i+1], second[i+2], second[i+3]), fill=(255, 255, 0), width=10)

    # save the image and display it
    im.save('pillow_imagedraw.jpg', quality=95)
    img = Image.open('pillow_imagedraw.jpg')
    img.show()
